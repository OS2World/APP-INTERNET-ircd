...
#else
extern	int	dn_skipname (__const__ u_char *, __const__ u_char *);
extern	int	dn_expand(__const__ u_char *, __const__ u_char *, __const__ u_char *,
			u_char *, int);
extern	int	res_mkquery(int, __const__ char *, int, int, __const__ char *, int,
...
#endif
{
	...
	if (a && rptr->type == T_PTR)
		...
		if(!rptr->he.h_name)
		{
			...
			sendto_realops ("WARNING: Error in lookup of %s. Tell darkrot.", badip);
			sendto_serv_butone(&me, ":%s GLOBOPS :Error in lookup of %s. Tell darkrot.",
				me.name, badip);
